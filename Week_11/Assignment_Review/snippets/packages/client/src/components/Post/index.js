...
const Post = ({
  ...
  const handleToggleLike = async () => {
    if (!likedState) {
      ...
      try {
        const response = await axios.post(`posts/like/${_id}`);
      } catch (error) {
        console.log(error);
        return error;
      }
  ...
  const OverlayTooltip = (props) => {
    return likes.length === 0 ? (
      ...
      <Tooltip {...props}>
        {likes
          .filter((l, i) => (likes.length <= 5 ? i <= i : i < 3))
          .map((l) => l.username)
          .join(", ")}
        {likes.length > 5 ? `, and ${likes.length - 3} more` : ""}
      </Tooltip>
    );
  };
            ...
            <Media.Body className="w-100">
              <div className="d-flex align-items-center">
                <Link
                  ...
                  className="text-muted mr-1 username no-underline"
                >
                  @{author.username}
                </Link>
              ...
              {stateComments.map((c, index) => (
                ...
                >
                  ...
                  >
                    <Figure.Image
                      src={
                        c.author === user._id || c.author?._id === user._id
                          ? user.profile_image
      ...
      >
        ...
        <Modal.Body className="d-flex justify-content-evenly">
          ...
          <Button variant="danger" onClick={handleDeletePost}>
            {data.isSubmitting ? <LoadingSpinner /> : "Confirm"}
          </Button>
        </Modal.Body>
      </Modal>
    </>
